...
iptables-save > /etc/iptables/gab-rules.v4
# extra iptables options if have slowdns
cat <<'LAMISA' > /etc/gab/slowdns-iptables.sh
#!/bin/bash
iptables-restore < /etc/iptables/gab-rules.v4
iptables -I INPUT -p udp --dport 5300 -j ACCEPT
iptables -t nat -I PREROUTING -p udp --dport 53 -j REDIRECT --to-ports 5300
iptables -A IN_public_allow -p tcp -m tcp --dport 10000 -m conntrack --ctstate NEW,UNTRACKED -j ACCEPT
iptables-save > /etc/iptables/rules.v4
LAMISA
chmod +x /etc/gab/slowdns-iptables.sh
